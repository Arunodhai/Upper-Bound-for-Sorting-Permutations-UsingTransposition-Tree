...
Original file is located at
    https://colab.research.google.com/drive/1z3pCQbu_x3Sm68viqTn654enKb6l779H
"""
...
def bfs(adjacency_matrix, node_count):
    ...
    algoG(adjacency_matrix, node_count,distsum,dist)
...
def algoG(adjacency_matrix, node_count,distsum,dist):
    gamma=0
    ...
    if (checkStar(adjacency_matrix, nodecount_temp,node_count)):    #checks if the given tree is a star graph.
        ...
        while (no > 0):
            ...
            dele = distsum.index(max1)
            
            
            Ecc = 0
            for x in range(1,tno+1):       #finding the maximum eccentricity
                Ecc = max(Ecc, dist[dele][x])
                
            gamma = gamma + Ecc

            distsum[dele]=0    #delete the node with maximum distance sum

            for x in range(1,tno+1):  #making all its entries invalid in the distance matrix, updating the distsum and degree_count
                if (not (x == dele) and not (distsum[x] == 0)):
                    if (x < dele):
                        distsum[x] = distsum[x] - dist[x][dele]
                    else:
                        distsum[x] = distsum[x] - dist[x][dele]
            ...
            for x in range(1,tno+1):
                ...
                adjacency_matrix[dele-1][x-1] = 0
                adjacency_matrix[x-1][dele-1] = 0
