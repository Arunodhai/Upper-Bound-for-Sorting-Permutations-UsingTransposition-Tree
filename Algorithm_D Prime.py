# -*- coding: utf-8 -*-
"""ALGO_D_PRIME_FINAL.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1zTpX9ihwDFpIfhWFpEGFvPAxes4LHmtP
"""
...
def bfs(adjacency_matrix, node_count):
  ...
  algoDprime(adjacency_matrix, node_count,distsum,dist)
...
def algoDprime(adjacency_matrix, node_count,distsum,dist):
   ...
   if (checkStar(adjacency_matrix, nodecount_temp,node_count)):    #checks if the given tree is a star graph.
        ...
        no=node_count-1 #while loop has to work only n-1 times
        while (no > 0):
          ...
          if len(C) > (2*len(S))/3 :
            if len(S)%2==0:
              ...
              for y in S :
                  ...
                  gamma =gamma + (len(S)*(diam-1/2)-1/2)
                  tno=nodecount_temp
                  for y in S :
                      distsum[y]=0
                      for x in range(1,tno+1):  #making all its entries invalid in the distance matrix, updating the distsum and degree_count
